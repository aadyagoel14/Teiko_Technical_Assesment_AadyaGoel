...
Analyzes immune cell population data from Bob Loblaw's miraclib clinical trial: a relational SQLite schema, a loading script, statistical comparisons of responders vs. non-responders, and an interactive dashboard.

**Live dashboard:** https://aadyagoel14-teiko-technical-assesment-aadyagoe-dashboard-pjmfdz.streamlit.app/
---
...
1. `condition`, `age`, `sex`, `treatment`, and `response` describe the
...
2. `sample_type` and `time_from_treatment_start` vary per blood draw, so they
  live on `samples`, one row per draw.
3. Cell counts are stored **long-format** (one row per sample × population)
...
1. **A new assay type or measurement kind** (e.g. cytokine levels alongside
...
2. **Analyses that need more subject-level detail** (e.g. treatment start
...
1. **Boxplot** (responders vs. non-responders), rendered as a static PNG via
  `run_analysis.py` and interactively (with individual points) in the
  dashboard.
2. **Mann-Whitney U test per population**, run on **one aggregated value per
...
3. **Benjamini-Hochberg FDR correction** across the 5 simultaneous tests, so
